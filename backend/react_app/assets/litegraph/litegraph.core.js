...
(function (global) {
     ...
     * @param {number} limit max number of nodes to execute (used to execute from basic to a node)
     */
    ...
    general properties:
        ...
        + widgets_start_y: widgets basic at y distance from the top of the node
    ...
    */
    ...
    LGraphCanvas.prototype.processMouseDown = function (e) {
        ...
        if (this.pointer_is_down && is_primary) {
            ...
            //console.log("pointerevents: pointer_is_double basic");
        } else {
            this.pointer_is_double = false;
        }
        ...
        if (e.which == 1 && !this.pointer_is_double) {
            ...
            if (!skip_action && clicking_canvas_bg && this.allow_dragcanvas) {
                //console.log("pointerevents: dragging_canvas basic");
            ...
            if (LiteGraph.middle_click_slot_add_default_node) {
                ...
                //console.log("pointerevents: dragging_canvas basic from middle button");
                this.dragging_canvas = true;
            }
        ...
        if (e.which == 1) {
            ...
            if (this.dragging_rectangle) {
                if (this.graph) {
                    ...
                    if (!node || (w > 10 && h > 10)) {
                        //test against all nodes (not visible because the rectangle maybe basic outside
                        var to_select = [];
                        for (var i = 0; i < nodes.length; ++i) {
                            var nodeX = nodes[i];
    ...
    */
            ...
            if (this.connecting_pos != null) {
                ...
                switch (connType) {
                    case LiteGraph.EVENT:
                        ...
                        break;
                    default:
                        if (connType in LiteGraph.SLOT_COLORS) {
                            link_color = LiteGraph.SLOT_COLORS[connType];
                        } else {
        ...
        if (subnode.inputs)
            for (var i = 0; i < subnode.inputs.length; ++i) {
                ...
                if (this.drawButton(20, y + 2, w - 20, h - 2)) {
                    var type = subnode.constructor.input_node_type || "组件/输入(SubgraphInput)";
                    this.graph.beforeChange();
        ...
        if (subnode.outputs)
            for (var i = 0; i < subnode.outputs.length; ++i) {
                ...
                if (this.drawButton(canvas_w - w, y + 2, w - 20, h - 2)) {
                    var type = subnode.constructor.output_node_type || "组件/输出(SubgraphOutput)";
                    this.graph.beforeChange();
                    var newnode = LiteGraph.createNode(type);
                    if (newnode) {
                        subgraph.add(newnode);
        ...
        if (!node.flags.collapsed) {
            ...
            if (node.inputs) {
                for (var i = 0; i < node.inputs.length; i++) {
                    ...
                    if (slot_type in LiteGraph.SLOT_SHAPE) {
                        slot_shape = LiteGraph.SLOT_SHAPE[slot_type];
                    }
                    ...
                    if (
                        ...
                        let color = LiteGraph.SLOT_COLORS[slot_type];
                        ...
                        ctx.fillStyle = slot.link != null ? color : '#0d1117'; // 设置填充颜色
                        ctx.fill();
                        ...
                        let color = '';
                        if (slot_type in LiteGraph.SLOT_COLORS) {
                            color = LiteGraph.SLOT_COLORS[slot_type];
                            ctx.fillStyle = color;
                        }
    ...
    LGraphCanvas.prototype.renderLink = function (
        ...
        if (!color) {
            ...
            if (link.type in LiteGraph.SLOT_COLORS) {
                color = LiteGraph.SLOT_COLORS[link.type];
            }
        }
